...
//    private static UnityEngine.Vector3 _terrainPos;
...
//        meshScale = new UnityEngine.Vector3(meshScale.x / (w - 1) * tRes, meshScale.y, meshScale.z / (h - 1) * tRes);
//        var uvScale = new UnityEngine.Vector2(1.0f / (w - 1), 1.0f / (h - 1));
//        var tData = _terrain.GetHeights(0, 0, w, h);

//        w = (w - 1) / tRes + 1;
//        h = (h - 1) / tRes + 1;
//        var tVertices = new UnityEngine.Vector3[w * h];
//        var tUV = new UnityEngine.Vector2[w * h];
...
//            for (var x = 0; x < w; x++)
//            {
//                tVertices[y * w + x] = UnityEngine.Vector3.Scale(meshScale, new UnityEngine.Vector3(-y, tData[x * tRes, y * tRes], x)) + _terrainPos;
//                tUV[y * w + x] = UnityEngine.Vector2.Scale(new UnityEngine.Vector2(x * tRes, y * tRes), uvScale);
//            }
//        }
